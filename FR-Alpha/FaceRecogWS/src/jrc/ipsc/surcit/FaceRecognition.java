...
public class FaceRecognition
{
	...
	{
		if (!imFnm.endsWith(FILE_EXT)) {
			System.out.println("Input image must be a "+FILE_EXT+" file");
			return null;
		}
		else
			System.out.println("Matching " + imFnm);
		
		BufferedImage image = FileUtils.loadImage(FileUtils.getFilesPath()+"\\"+imFnm);
		if (image == null)
			return null;
	...
	public static JSONObject mainRecog(String[] args)
	{
		...
		if ((args.length < 1) || (args.length > 2)) {
			jsonERROR.put("ERROR", "-2");
			jsonERROR.put("ERRORMessage", "Usage: java FaceRecognition imagePngFnm [numberOfEigenfaces]");
			System.out.println("Usage: java FaceRecognition imagePngFnm [numberOfEigenfaces]");
			return jsonResponse;
		}
		...
		if (args.length == 2) {
			...
			{ 
				...
				jsonERROR.put("ERRORMessage", "Number argument, " + args[1] + " must be an integer");
				}
		}
		...
		if (result == null)
		{
			...
			jsonERROR.put("ERRORMessage", "No match found");
		}
		else 
		{
			jsonERROR.put("ERROR", "0");
			jsonRESULT.put("message", "Matches image in " + result.getMatchFileName());
			jsonRESULT.put("distance", Double.toString(result.getMatchDistance()));
